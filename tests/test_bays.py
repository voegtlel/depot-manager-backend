...
def test_bay(monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        resp = client.get('/api/v1/bays', auth=MockAuth(sub='user1'))
        assert resp.status_code == 200, resp.text
        bays = [Bay.validate(b) for b in resp.json()]
        assert len(bays) == 1
        assert bays[0] == created_bay

        resp = client.get(f'/api/v1/bays/{created_bay.id}', auth=MockAuth(sub='user1'))
        assert resp.status_code == 200, resp.text
        ...
        resp = client.put(
            f'/api/v1/bays/{created_bay.id}', data=update_bay.json(), auth=MockAuth(sub='admin1', roles=['admin'])
        )
        assert resp.status_code == 200, resp.text
        ...
        resp = client.delete(
            f'/api/v1/bays/{created_bay.id}', auth=MockAuth(sub='admin1', roles=['admin'])
        )
        assert resp.status_code == 200, resp.text
